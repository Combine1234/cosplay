...
export function createStudioRenderer(host,onStatus=()=>{}){
 ...
 function fitItem(object,item,ref){const current=modelState.body,base=item.model.referenceBody||BODY_DEFAULTS.female,variant=item.sizeVariants.find(v=>v.id===ref.variantId),medium=item.sizeVariants.find(v=>v.size==='M')||item.sizeVariants[0],t=ref.transform;
  ...
  if(!accessory)object.traverse(mesh=>{if(!mesh.isMesh)return;const a=mesh.geometry.attributes.position,r=mesh.userData.rest;for(let i=0;i<a.count;i++)a.setXYZ(i,(r[i*3]-anchor[0])*sx*clearance,r[i*3+1]-anchor[1],(r[i*3+2]-anchor[2])*depth);a.needsUpdate=true;mesh.geometry.computeVertexNormals();mesh.geometry.computeBoundingSphere();});
